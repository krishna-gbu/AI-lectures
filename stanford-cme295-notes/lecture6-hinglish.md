...
R1-Zero
  -> long CoTs / reasoning traces generate
  -> humans clean/rewrite

V3-Base + cleaned reasoning data
  -> small SFT
  -> GRPO with reasoning data
...
Clarification:
- isse aisa mat samjho ki `R1-Zero` ko simply dubara pretrain kiya gaya
- better reading ye hai ki `R1-Zero` ne reasoning behavior prove kiya aur useful reasoning traces diye, jinse `R1` ka hybrid pipeline build hua
...
Important reading:
- yahan point ye nahi ki `R1-Zero` checkpoint ko dubara pretrain kiya gaya
- notes ke flow me `V3-Base` starting base hai, aur `R1-Zero` mainly reasoning-data / proof-of-concept source ki tarah help karta hai
...
- student teacher ke next-token probabilities imitate karta hai
- simple example: teacher next word ke liye `Paris` ko high probability deta hai, student bhi waise distribution learn kare

**Yahan use hui reasoning distillation**
- teacher: `R1`
- student: `R1-Distill`
- teacher full responses / reasoning traces generate karta hai
- student SFT ke through un reasoning traces ko learn karta hai
- simple example: teacher math problem me steps likhkar final answer deta hai, student wahi step-by-step solve pattern learn karta hai

Slide ka contrast:
- old goal: `match next token distribution`
- new goal: `learn full reasoning traces through SFT`

Results slide message:
- distilled models competitive ho sakte hain
- ye compute ka "good" use ho sakta hai

**Short distillation recall:**

| Style | Simple meaning | Example |
|---|---|---|
| Classic distillation | teacher ke token probabilities copy karo | next word probability imitate karo |
| Reasoning distillation | teacher ka full solve pattern copy karo | step-by-step solution SFT se seekho |
