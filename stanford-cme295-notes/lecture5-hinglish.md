...
**Short trainer-vs-user view:**

| View | Flow | Meaning |
|---|---|---|
| Normal user | `prompt -> improved policy -> response` | user ko sirf final aligned model dikhta hai |
| Model trainer | `prompt -> policy response -> reward model score + reference-model KL check + value estimate -> PPO update -> improved policy` | yahi RLHF ka actual training loop hai |

Role recall:
- `Reward model`: response ko score karta hai
- `Reference model`: base/SFT se drift control karta hai
- `Value model`: expected reward baseline estimate karta hai
- `PPO`: small stable policy updates karta hai
...
**Short PPO-Clip vs PPO-KL recall:**
